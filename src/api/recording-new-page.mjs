...
<main>
  ...
  <div class="panel">
    ...
    <p class="hint">Optional. Add values only if your starting tab URLs use templates like <strong>{statementMonth}</strong>, <strong>{accountId}</strong>, or <strong>{{recordId}}</strong>. Browsy resolves these before opening the browser.</p>
...
function hasTemplateVars(value){return /\{\{[^{}]+\}\}|\{[^{}]+\}/.test(String(value||''))}
function resolveTemplate(value, vars){return String(value||'').replace(/\{\{([^{}]+)\}\}|\{([^{}]+)\}/g,function(match,a,b){const key=String(a||b||'').trim();if(vars[key]===undefined||vars[key]===null||vars[key]==='')throw new Error('Missing URL parameter: '+key);return String(vars[key])})}
function makePayload(){syncTabs();syncParams();const appId=safeId(el('appId').value,'general-automation');const workflowId=safeId(el('workflowId').value,'recorded-workflow');const vars=paramsObject();const cleanTabs=tabs.map(function(tab,index){const id=safeId(tab.id,'tab'+(index+1));const rawUrl=tab.url.trim();const url=hasTemplateVars(rawUrl)?resolveTemplate(rawUrl,vars):rawUrl;const out={id,title:tab.title||id,url,siteId:safeId(tab.siteId||id,id),requiresAuth:!!tab.requiresAuth};if(tab.authProfileId||tab.requiresAuth)out.authProfileId=safeId(tab.authProfileId||tab.siteId||id,id);return out}).filter(function(tab){return tab.url});if(!cleanTabs.length)throw new Error('Add at least one starting tab URL.');const targetUrl=cleanTabs[0].url;return{appId,appName:el('appName').value.trim()||appId,workflowId,workflowName:el('workflowName').value.trim()||workflowId,targetUrl,recordingSetup:{tabs:cleanTabs},payloadSchema:{type:'object',additionalProperties:true,properties:{},required:[]},fieldContractIntent:'General Browsy recording created from /recordings/new. Operator supplied only starting tabs, auth requirements, and URL parameters. Browser observation records the workflow.',completionPolicy:{action:'wait_for_human_submission',notes:'Workflow details are captured from browser observation.'}}}
