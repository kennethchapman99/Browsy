...
async function runAction(label,fn){if(actionInFlight)return;actionInFlight=true;const busyText=label+'...';el('actionResult').className='result status warn';el('actionResult').textContent=busyText;el('contractOutput').textContent='{}';validateSetup();try{const result=await fn();if(el('actionResult').textContent===busyText){el('actionResult').className='result status ok';el('actionResult').textContent=''}return result}catch(err){const msg='Error: '+(err.message||String(err));el('actionResult').className='result status bad';el('actionResult').textContent=msg;el('contractOutput').textContent=JSON.stringify({error:err.message||String(err),details:err.data||null},null,2)}finally{actionInFlight=false;validateSetup()}}
