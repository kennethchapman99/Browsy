...
async function runAction(label,fn){if(actionInFlight)return;actionInFlight=true;el('actionResult').className='result status warn';el('actionResult').textContent=label+'...';el('contractOutput').textContent='{}';validateSetup();try{const result=await fn();return result}catch(err){const msg='Error: '+(err.message||String(err));el('actionResult').className='result status bad';el('actionResult').textContent=msg;el('contractOutput').textContent=JSON.stringify({error:err.message||String(err),details:err.data||null},null,2)}finally{actionInFlight=false;validateSetup()}}
