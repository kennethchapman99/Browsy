...
export function normalizeObservation(input = {}) {
  ...
  const regularFields = normalizeFields(raw.fields || raw.globalFields || [], { defaultScope: 'global' });
  const globalAssets = normalizeFields(raw.globalAssets || [], { defaultScope: 'asset', defaultInputType: 'file' })
    .map(asset => ({ ...asset, inputType: 'file', scope: 'asset' }));
  ...
  return {
    ...
    fields: [...regularFields, ...globalAssets],
    actions,
    repeatGroups,
...
export function buildWorkflowPackageFromObservation(observation) {
  ...
  const capturedOutputs = materialized.outputs.map(output => cleanObject({
    ...
    attribute: output.attribute || null,
  ...
  for (const group of repeatGroups) {
    const itemAssets = group.items?.[0]?.assets || {};
  ...
  pkg.assets = assetEntries;
  if (!pkg.capture_outputs) pkg.capture_outputs = [];
...
export function buildWorkflowConfigFromObservation(observation) {
  ...
  return cleanObject({
    ...
    humanCheckpoints: materialized.checkpoints,
    manualOnlyActions: inferManualOnlyActions(obs),
    safetyPolicy: materialized.safetyPolicy,
    tabs: materialized.tabs,
    auth: materialized.auth,
...
function inferObservedCheckpoints(obs, actions) {
  ...
  for (const action of actions || []) {
    if (action.manualOnly || action.dangerous || DANGEROUS_WORDS.test(action.label || '')) checkpoints.push(cleanObject({ id: `${action.id}Approval`, label: `Review before ${action.label || action.id}`, beforeAction: action.id, reason: action.safetyCategory || 'manual approval required', sourceEventId: action.sourceEventId }));
  }
...
function normalizePage(page = {}, index = 0) {
  ...
  return cleanObject({ id, purpose: page.purpose || page.label || page.name || id, url: page.url || page.startUrl || '', urlTemplate: page.urlTemplate || page.templateUrl || (hasTemplateVars(page.url || '') ? page.url : ''), exampleUrl: page.exampleUrl || page.example_url || '', captureAfter: page.captureAfter || page.afterAction || '', captureRegex: page.captureRegex || page.regex || '', notes: page.notes || '' });
}
...
function normalizeRepeatGroups(groups = [], actions = []) {
  return asArray(groups).map(group => {
    ...
    return cleanObject({ id, label: group.label || humanize(id), itemLabel, itemPlural: group.itemPlural || pluralize(itemLabel), source: group.source || null, addAction: addActionRaw ? normalizeAction(addActionRaw) : null, itemFields: normalizeFields(group.itemFields || group.fields || [], { defaultScope: 'item' }).map(f => ({ ...f, scope: 'item' })), itemAssets: normalizeFields(group.itemAssets || group.assets || [], { defaultScope: 'item_asset', defaultInputType: 'file' }).map(f => ({ ...f, inputType: 'file', scope: 'item_asset' })) });
  });
}
...
function normalizeAction(action = {}) {
  ...
  return cleanObject({ id, label, type: action.type || 'click', scope: action.scope || '', targetRepeatGroup: action.targetRepeatGroup || action.repeatGroupId || '', selectorHint: action.selectorHint || action.selector || '', selectorCandidates: action.selectorCandidates || [], selectorConfidence: action.selectorConfidence || '', tabId: action.tabId || action.pageId || '', sourceEventId: action.sourceEventId || '', manualOnly, dangerous: action.dangerous === true || manualOnly, safetyCategory, notes: action.notes || '' });
}
...
function normalizeCapturedOutputList(outputs = []) {
  return asArray(outputs).map(output => {
    ...
    return cleanObject({ id, label: output.label || output.name || humanize(id), scope: output.scope || inferOutputScope(id), source: output.source || 'captured_from_page', regex: output.regex || '', selector: output.selector || '', attribute: output.attribute || '', required: output.required !== false, captureAfter: output.captureAfter || output.afterAction || '', example: output.example || output.exampleValue || '', verify: output.verify || null, storesTo: output.storesTo || `captured.${id}` });
  });
}
...
function inferManualOnlyActions(obs) { return dedupeById(obs.actions.filter(action => action.manualOnly || action.dangerous || action.safetyCategory)); }
function buildSampleRepeatItem(group) { const fields = {}; const assets = {}; for (const field of group.itemFields || []) fields[field.id] = field.exampleValue ?? placeholderValue(field); for (const asset of group.itemAssets || []) assets[asset.id] = asset.exampleValue ?? placeholderValue(asset); return cleanObject({ fields, assets }); }
function buildSafetyPolicy(manualOnlyActions) { const neverClickText = [...new Set(manualOnlyActions.map(a => a.label).filter(Boolean))]; const neverClickSelectors = [...new Set(manualOnlyActions.map(a => a.selectorHint || a.selector).filter(Boolean))]; return { never_click_text: neverClickText, never_click_selectors: neverClickSelectors, manual_only_categories: ['final submission', 'payment', 'legal certification', 'destructive action'], dry_run_default: true, pause_at_end_default: true }; }
function placeholderValue(field) { if (field.inputType === 'file') return `./${field.id}`; if (/date/i.test(field.id)) return '2099-01-01'; return `Example ${field.label || humanize(field.id)}`; }
function regexForUrlTemplate(template, varName) { if (!template || !template.includes(`{{${varName}}}`)) return ''; const before = template.split(`{{${varName}}}`)[0] || ''; const marker = before.match(/\/([^/?#/]*)$/)?.[1] || ''; const prefix = marker ? `/${escapeRegex(marker)}` : ''; return `${prefix}/([A-Za-z0-9_-]+)`; }
function inferOutputScope(value = '') { return /url|link|page|public/i.test(String(value)) ? 'external_link' : 'captured'; }
function normalizeInputType(type = 'text') { const t = String(type || 'text').toLowerCase(); return FILE_INPUT_TYPES.has(t) || t.includes('file') || t.includes('upload') ? 'file' : t; }
function extractTemplateVars(text = '') { const vars = []; const re = /\{\{([^}]+)\}\}/g; let match; while ((match = re.exec(String(text))) !== null) vars.push(match[1].trim()); return [...new Set(vars)]; }
function hasTemplateVars(text = '') { return /\{\{[^}]+\}\}/.test(String(text)); }
function toWorkflowId(value = '') { return String(value || 'workflow').toLowerCase().replace(/[^a-z0-9-_]+/g, '-').replace(/^-+|-+$/g, '') || 'workflow'; }
function toCamel(value = '') { const parts = String(value || '').replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[^a-zA-Z0-9]+/g, ' ').trim().split(/\s+/).filter(Boolean); return parts.map((part, index) => { const lower = part.toLowerCase(); return index === 0 ? lower : lower.charAt(0).toUpperCase() + lower.slice(1); }).join('') || 'field'; }
function humanize(value = '') { return String(value || '').replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[-_]+/g, ' ').replace(/\s+/g, ' ').trim().replace(/^./, c => c.toUpperCase()); }
function pluralize(value = '') { const v = String(value || 'item'); if (/s$/i.test(v)) return v; if (/y$/i.test(v)) return v.slice(0, -1) + 'ies'; return v + 's'; }
function singularize(value = '') { const v = String(value || 'item'); if (/ies$/i.test(v)) return v.slice(0, -3) + 'y'; if (/s$/i.test(v) && v.length > 1) return v.slice(0, -1); return v; }
function firstSelector(input = {}) { return asArray(input.selectorCandidates || input.rawEvidence?.selectorCandidates)[0]?.selector || ''; }
function asArray(value) { return Array.isArray(value) ? value : value ? [value] : []; }
function dedupeById(items) { const map = new Map(); for (const item of items) if (item?.id) map.set(item.id, item); return [...map.values()]; }
function dedupeByName(items) { const map = new Map(); for (const item of items) if (item?.name) map.set(item.name, item); return [...map.values()]; }
function cleanObject(obj) { if (Array.isArray(obj)) return obj.map(cleanObject); if (!obj || typeof obj !== 'object') return obj; const out = {}; for (const [key, value] of Object.entries(obj)) { if (value === undefined || value === null || value === '') continue; if (Array.isArray(value) && value.length === 0) continue; out[key] = cleanObject(value); } return out; }
function stripEmpty(obj) { const out = {}; for (const [key, value] of Object.entries(obj)) { if (value === undefined || value === null || value === '') continue; if (Array.isArray(value) && value.length === 0) continue; if (value && typeof value === 'object' && !Array.isArray(value) && Object.keys(value).length === 0) continue; out[key] = value; } return out; }
