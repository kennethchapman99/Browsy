...
1. Log in manually once:
   ```bash
   npm run auth:save -- --workflow test-form --url <START_URL>
   ```
2. Verify the session was saved:
   ```bash
   npm run auth:check -- --workflow test-form --url <START_URL>
   ```
...
```bash
npm run discover -- --workflow test-form --url <START_URL> --candidates
```

Artifacts written to: `output/runs/test-form/<timestamp>/`

Review `field-map.candidates.md`, then create `workflows/test-form/field-map.local.json` with verified selectors.

## Dry-run

```bash
npm run run -- --workflow test-form --manifest workflows/test-form/manifest.example.json --dry-run
```

## Expected artifacts

Every run writes to `output/runs/test-form/<timestamp>/`:
